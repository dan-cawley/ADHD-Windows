...
namespace AdhdWarrior {
 public static class JourneyTests {
  ...
  public static int Run(string folder) {
   ...
   var migrated=Storage.Decode(old);Expect(migrated.Version==4&&migrated.XP==200&&migrated.Coins==40&&migrated.Journey.Pets.Count==1,"V1 migration preserves balances and grants exactly one starter egg");
