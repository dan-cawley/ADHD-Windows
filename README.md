# ADHD Warrior for Windows — 0.8

Native Windows desktop preview with the quest loop and adventure progression.

## Run

Open **Launch ADHD Warrior.lnk** in this folder, or **dist/0.8/ADHD Warrior.exe**. Close any older ADHD Warrior window first. Keep the config and assets folder with the executable. Requires .NET Framework 4.8.

The earlier executables remain under `dist/` for reference. Use 0.8 for normal work. Versions 0.6–0.8 share save format 5; older previews cannot read it.
...
Run `powershell -NoProfile -ExecutionPolicy Bypass -File .\build.ps1`. The included Windows .NET Framework compiler builds into `dist/0.8/`; no package downloads are required. A Visual Studio project is also provided.

Run `Start-Process -FilePath '.\dist\0.8\ADHD Warrior.exe' -ArgumentList '--self-test' -WindowStyle Hidden -Wait -PassThru`. Exit code 0 means success. Results are in `dist/0.8/test-results.txt`. Regression tests use isolated temporary data.
