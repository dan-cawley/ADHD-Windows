...
Writes are atomic. The prior file is retained as `save.json.bak`. Windows save formats 1–6 migrate to format 7 when loaded and are written in the new format after the next successful change. Invalid or unsupported saves stop loading before the original is overwritten.
