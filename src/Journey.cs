...
namespace AdhdWarrior {
 ...
 public static class Journey {
  ...
  public static void OnCompletion(SaveData data,Quest quest,DateTime day) {
   ...
   if(j.BossHP==0){AddHistory(j,"Defeated");Log(j,"Defeated "+Bosses[j.BossIndex]+"!");AwardGear(data,true);j.BossIndex=(j.BossIndex+1)%Bosses.Length;j.BossMaxHP=TargetHP(j);j.BossHP=j.BossMaxHP;}
   if(j.Completions%6==0)AwardGear(data,false);
  ...
  public static void BuyGear(SaveData data,string id){var g=GearCatalog.All.Single(x=>x.Id==id);if(data.Journey.Gear.Contains(id))throw new InvalidOperationException("You already own this item.");if(data.PendingRewards.Any(r=>r.GearId==id))throw new InvalidOperationException("This item is already waiting in Rewards.");if(data.Coins<g.Price)throw new InvalidOperationException("Not enough coins.");data.Coins-=g.Price;data.Journey.Gear.Add(id);Log(data.Journey,"Purchased "+g.Name+".");}
