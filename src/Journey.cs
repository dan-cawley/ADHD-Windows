...
namespace AdhdWarrior {
 ...
 public static class Journey {
  ...
  public static void OnCompletion(SaveData data,Quest quest,DateTime day) {
   ...
   if(j.BossHP==0){AddHistory(j,"Defeated");Log(j,"Defeated "+Bosses[j.BossIndex]+"!");AwardNextGear(data,"Boss victory");j.BossIndex=(j.BossIndex+1)%Bosses.Length;j.BossMaxHP=TargetHP(j);j.BossHP=j.BossMaxHP;}
   if(j.Completions%6==0)AwardNextGear(data,"Six completions");
   if(p.EggStage==4&&p.LootSkill>0&&LootRoll.Next(100)<Math.Min(100,p.LootSkill*Stage(p)))AwardNextGear(data,"Familiar bonus loot");
  }
  public static GearDefinition AwardNextGear(SaveData data,string source){var avatar=Identity.CurrentJourneyAvatar(data);var item=GearCatalog.All.Where(g=>g.Sheet==avatar.Id&&!data.Journey.Gear.Contains(g.Id)).OrderBy(g=>g.Tile).FirstOrDefault();if(item==null){Log(data.Journey,"Every avatar collection is complete. Your adventure continues!");return null;}data.Journey.Gear.Add(item.Id);Log(data.Journey,source+": "+item.Name+" was added to your "+avatar.Title+" collection.");var next=Identity.CurrentJourneyAvatar(data);if(next.Id!=avatar.Id){data.AvatarSet=next.Id;Log(data.Journey,avatar.Title+" is complete. Your "+next.Title+" journey begins!");}return item;}
  ...
  public static void BuyGear(SaveData data,string id){var g=GearCatalog.All.Single(x=>x.Id==id);if(g.Sheet!=Identity.CurrentJourneyAvatar(data).Id)throw new InvalidOperationException("Finish the current avatar collection first.");if(data.Journey.Gear.Contains(id))throw new InvalidOperationException("You already own this item.");if(data.Coins<g.Price)throw new InvalidOperationException("Not enough coins.");data.Coins-=g.Price;data.Journey.Gear.Add(id);var next=Identity.CurrentJourneyAvatar(data);if(next.Id!=g.Sheet)data.AvatarSet=next.Id;Log(data.Journey,"Purchased "+g.Name+" for your character collection.");}
  ...
  public static void Train(SaveData data,string id,string skill){var p=data.Journey.Pets.Single(x=>x.Species==id);if(p.EggStage<4||p.Points<1)throw new InvalidOperationException("Hatch your familiar and earn a skill point first.");if(skill=="Quest XP")p.QuestSkill++;else if(skill=="Streak XP")p.StreakSkill++;else if(skill=="Loot Chance")p.LootSkill++;else throw new InvalidOperationException("Unknown familiar skill.");p.Points--;}
