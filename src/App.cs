...
namespace AdhdWarrior {
 ...
 public partial class MainWindow : Form {
  ...
  public MainWindow(bool testMode=false) {
   ...
   data=Storage.Load(path); Journey.RefreshWeek(data,DateTime.Today); Text="ADHD Warrior — Windows 0.11"+(testMode?" [TEST DATA]":""); MinimumSize=new Size(1000,680); Size=new Size(1240,860); StartPosition=FormStartPosition.CenterScreen; Font=new Font("Segoe UI",10); BackColor=paper; ForeColor=ink; AutoScaleMode=AutoScaleMode.Dpi;
  ...
  void Render() {
   ...
   if(view=="Character")ShowCharacter();
    ...
    Note("Save location:\n"+path+"\n\nThe previous save is retained as save.json.bak.");Note("Backups use Windows format 6. Older Windows backups upgrade automatically. iOS import supports quest due times and active legacy daily recurrence in addition to adventure progress.\n\nKeyboard: Enter to capture; Tab to move between controls; Space to select.");
   } else {
