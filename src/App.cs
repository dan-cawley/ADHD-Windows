...
namespace AdhdWarrior {
 static class Program {
  [STAThread] static int Main(string[] args) {
   ...
   bool first; using(var mutex=new Mutex(true,"Local\\AdhdWarriorWindows",out first)) {
 ...
 public class MainWindow : Form {
  ...
  public MainWindow(bool testMode=false) {
   ...
   data=Storage.Load(path); Text="ADHD Warrior — Windows"; MinimumSize=new Size(1000,680); Size=new Size(1200,820); StartPosition=FormStartPosition.CenterScreen; Font=new Font("Segoe UI",10); BackColor=paper; ForeColor=ink; AutoScaleMode=AutoScaleMode.Dpi;
   ...
   var nav=new FlowLayoutPanel {Dock=DockStyle.Fill,FlowDirection=FlowDirection.TopDown,WrapContents=false,Padding=new Padding(18,28,12,12),BackColor=Color.FromArgb(231,235,225)};layout.Controls.Add(nav,0,0);
   nav.Controls.Add(new Label {Text="ADHD\nWARRIOR",Font=new Font("Segoe UI",21,FontStyle.Bold),AutoSize=false,Size=new Size(185,90)});
   nav.Controls.Add(new Label {Text="Small steps. Real progress.",AutoSize=true,Margin=new Padding(0,0,0,30)});
   foreach(string v in new[]{"Today","All quests","Review","Rewards","Completed","Archive","Backup & settings"}) {string target=v;var b=Button(v,()=>{view=target;selected.Clear();Render();});b.Width=182;b.Height=43;nav.Controls.Add(b);}
   var art=Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"assets","arcane_drake_egg.png");
   if(File.Exists(art)) nav.Controls.Add(new PictureBox {Image=Image.FromFile(art),SizeMode=PictureBoxSizeMode.Zoom,Size=new Size(180,145),Margin=new Padding(0,25,0,0)});
   ...
   var bar=new FlowLayoutPanel {Dock=DockStyle.Fill};search.Width=150;search.AccessibleName="Search quests";search.TextChanged+=(s,e)=>Render();bar.Controls.Add(search);bar.Controls.Add(Button("Complete selected",CompleteSelected));bar.Controls.Add(Button("Archive selected",ArchiveSelected));body.Controls.Add(bar,0,3);
  ...
  void Change(Action action,string message) {
   ...
   try {action();Storage.Save(path,data);status.Text=message;Render();} catch(Exception ex) {data=Storage.Decode(before);Render();MessageBox.Show(this,"Your change was not saved. "+ex.Message,"Save problem",MessageBoxButtons.OK,MessageBoxIcon.Error);}
  }
  void AddQuick() {var title=capture.Text.Trim();if(title.Length==0){capture.Focus();return;}Change(()=>data.Quests.Add(new Quest {Title=title,Due=DateTime.Today.ToString("yyyy-MM-dd")}),"Captured. One small step is enough.");capture.Clear();capture.Focus();}
  ...
  void CompleteSelected() {var list=Selection();if(list.Count==0){status.Text="Select one or more active quests first.";return;}int xp=list.Sum(q=>q.XP);Change(()=>Game.Complete(data,list,DateTime.Today),"Well done! +"+xp+" XP. Your progress is saved.");selected.Clear();}
  ...
  void Render() {
   ...
   if(view=="Rewards") {Note("Every step counts.\n\n"+data.Quests.Count(q=>q.Done)+" quests completed • "+data.XP+" lifetime XP • "+data.Coins+" coins earned");Note("Next level in "+(500-data.XP%500)+" XP.\n\nQuest rewards: XP you choose, plus 1 coin per 5 XP. Completing a quest also completes its remaining steps.");Note("Your original familiar, gear and boss artwork is preserved in this project.\nThese game systems are planned for the next migration phase.");}
   else if(view=="Backup & settings") {
    Note("Windows preview 0.1\n\nYour progress is saved on this computer after every change. No account is required.");
    cards.Controls.Add(Button("Export Windows backup…",Export));cards.Controls.Add(Button("Restore Windows backup…",Import));
    Note("Save location:\n"+path+"\n\nThe previous save is retained as save.json.bak.");Note("Backups use the Windows version 1 format. iOS and Android saves are not yet compatible.\n\nKeyboard: Enter to capture; Tab to move between controls; Space to select.");
   } else {
  ...
  void AddCard(Quest q,string today) {
   ...
   else if(!q.Done) {actions.Controls.Add(Button("Complete",()=>Change(()=>Game.Complete(data,new[]{q},DateTime.Today),"Well done! +"+q.XP+" XP and +"+(q.XP/5)+" coins.")));actions.Controls.Add(Button("Edit",()=>Edit(q)));if(overdue)actions.Controls.Add(Button("Move to today",()=>Change(()=>q.Due=today,"A fresh start. Quest moved to today.")));}
   else actions.Controls.Add(new Label {Text="Completed "+q.Completed,AutoSize=true});
