...
namespace AdhdWarrior {
 ...
 public class CategoryIcon : Control {
  public string Category="Life";public Color Accent=Theme.Emerald;
  public CategoryIcon(){SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw,true);TabStop=false;}
  protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;var r=new Rectangle(8,8,Math.Max(1,Width-17),Math.Max(1,Height-17));using(var wash=new SolidBrush(Color.FromArgb(35,Accent)))e.Graphics.FillEllipse(wash,r);using(var pen=new Pen(Accent,Math.Max(2,Width/28f))){pen.StartCap=LineCap.Round;pen.EndCap=LineCap.Round;float x=r.X,y=r.Y,w=r.Width,h=r.Height,cx=x+w/2,cy=y+h/2;if(Category=="Work"){var box=new RectangleF(x+w*.2f,y+h*.36f,w*.6f,h*.4f);e.Graphics.DrawRectangle(pen,box.X,box.Y,box.Width,box.Height);e.Graphics.DrawArc(pen,x+w*.38f,y+h*.2f,w*.24f,h*.28f,180,180);e.Graphics.DrawLine(pen,x+w*.2f,cy,x+w*.8f,cy);}else if(Category=="School"){PointF[] cap={new PointF(cx,y+h*.2f),new PointF(x+w*.84f,y+h*.4f),new PointF(cx,y+h*.58f),new PointF(x+w*.16f,y+h*.4f)};e.Graphics.DrawPolygon(pen,cap);e.Graphics.DrawArc(pen,x+w*.32f,y+h*.48f,w*.36f,h*.24f,0,180);e.Graphics.DrawLine(pen,x+w*.84f,y+h*.4f,x+w*.84f,y+h*.68f);}else if(Category=="Home"){e.Graphics.DrawLine(pen,x+w*.17f,cy,x+ w*.5f,y+h*.2f);e.Graphics.DrawLine(pen,x+w*.5f,y+h*.2f,x+w*.83f,cy);e.Graphics.DrawRectangle(pen,x+w*.27f,cy,w*.46f,h*.28f);e.Graphics.DrawRectangle(pen,x+w*.46f,cy+h*.1f,w*.13f,h*.18f);}else if(Category=="Fun"){PointF[] star=new PointF[10];for(int i=0;i<10;i++){double a=-Math.PI/2+i*Math.PI/5;float radius=i%2==0?w*.32f:w*.14f;star[i]=new PointF(cx+(float)Math.Cos(a)*radius,cy+(float)Math.Sin(a)*radius);}e.Graphics.DrawPolygon(pen,star);}else{using(var path=new GraphicsPath()){path.AddBezier(cx, y+h*.76f,x-w*.02f,y+h*.48f,x+w*.2f,y+h*.18f,cx,y+h*.4f);path.AddBezier(cx,y+h*.4f,x+w*.8f,y+h*.18f,x+w*1.02f,y+h*.48f,cx,y+h*.76f);e.Graphics.DrawPath(pen,path);}}} }
 }
