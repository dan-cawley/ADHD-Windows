...
namespace AdhdWarrior {
 ...
 public static class Identity {
  ...
  public static bool SetComplete(SaveData data,AvatarDefinition avatar){var items=GearCatalog.All.Where(g=>g.Sheet==avatar.Id).ToList();return items.Count>0&&items.All(g=>data.Journey.Gear.Contains(g.Id));}
  public static AvatarDefinition CurrentJourneyAvatar(SaveData data){return Avatars.FirstOrDefault(x=>!SetComplete(data,x))??Avatars.Last();}
  public static bool Unlocked(SaveData data,AvatarDefinition avatar){int index=Array.IndexOf(Avatars,avatar);return index>=0&&(SetComplete(data,avatar)||Avatars.Take(index).All(x=>SetComplete(data,x)));}
