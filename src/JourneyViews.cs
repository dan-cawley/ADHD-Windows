...
namespace AdhdWarrior {
 public partial class MainWindow {
  ...
  void ShowGear() {
   ...
   var items=GearCatalog.All.Where(g=>(gearShop?!data.Journey.Gear.Contains(g.Id)&&!data.PendingRewards.Any(r=>r.GearId==g.Id):data.Journey.Gear.Contains(g.Id))&&(gearSet=="All sets"||g.Sheet==gearSet)&&g.Name.IndexOf(search.Text,StringComparison.OrdinalIgnoreCase)>=0).ToList();
  ...
  void CompleteQuests(IEnumerable<Quest> quests){var list=quests.ToList();int xp=0,coins=list.Where(q=>!q.Done&&!q.Archived).Sum(q=>q.XP/5),bossIndex=data.Journey.BossIndex,bossMaxHP=data.Journey.BossMaxHP,history=data.Journey.History.Count;var owned=new HashSet<string>(data.Journey.Gear);if(Change(()=>{xp=Game.Complete(data,list,DateTime.Today);selected.Clear();},"Progress saved.")){status.Text="Well done! +"+xp+" XP · +"+coins+" coins · Your familiar and boss progressed.";bool defeated=data.Journey.History.Count>history&&data.Journey.History.Take(data.Journey.History.Count-history).Any(h=>h.Index==bossIndex&&h.Outcome=="Defeated");var rewards=data.Journey.Gear.Where(id=>!owned.Contains(id)).Select(id=>GearCatalog.All.Single(g=>g.Id==id).Name).ToList();using(var dialog=new QuestVictoryDialog(Journey.Bosses[bossIndex],Artwork(bossIndex<9?"weekly_monsters_1_9":"weekly_monsters_10_18",bossIndex%9),xp,bossMaxHP,defeated,data.Journey.BossHP,Journey.Bosses[data.Journey.BossIndex],rewards))dialog.ShowDialog(this);}}
 }
 public class QuestVictoryDialog : Form {
  public QuestVictoryDialog(string boss,Image artwork,int damage,int maxHP,bool defeated,int currentHP,string nextBoss,List<string> rewards){Text=defeated?"Boss defeated!":"Quest complete";Size=new Size(650,430);MinimumSize=Size;MaximumSize=Size;StartPosition=FormStartPosition.CenterParent;Font=new Font("Segoe UI",10);BackColor=Theme.Canvas;ForeColor=Theme.Text;ShowInTaskbar=false;var frame=new TableLayoutPanel {Dock=DockStyle.Fill,ColumnCount=2,RowCount=1,Padding=new Padding(24)};frame.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,220));frame.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));Controls.Add(frame);var portrait=new PictureBox {Dock=DockStyle.Fill,Image=artwork,SizeMode=PictureBoxSizeMode.Zoom,BackColor=Theme.Surface,Padding=new Padding(10),AccessibleName=boss+" artwork"};Theme.Frame(portrait,defeated?Theme.Gold:Theme.Coral);frame.Controls.Add(portrait,0,0);var details=new TableLayoutPanel {Dock=DockStyle.Fill,ColumnCount=1,RowCount=6,Padding=new Padding(24,4,0,0)};details.RowStyles.Add(new RowStyle(SizeType.Absolute,30));details.RowStyles.Add(new RowStyle(SizeType.Absolute,50));details.RowStyles.Add(new RowStyle(SizeType.Absolute,65));details.RowStyles.Add(new RowStyle(SizeType.Absolute,38));details.RowStyles.Add(new RowStyle(SizeType.Percent,100));details.RowStyles.Add(new RowStyle(SizeType.Absolute,52));details.Controls.Add(new Label {Text=defeated?"VICTORY":"QUEST COMPLETE",Dock=DockStyle.Fill,Font=new Font("Segoe UI",9,FontStyle.Bold),ForeColor=defeated?Theme.Gold:Theme.Emerald},0,0);details.Controls.Add(new Label {Text=boss,Dock=DockStyle.Fill,Font=new Font("Georgia",21,FontStyle.Bold),ForeColor=Theme.Text,AutoEllipsis=true},0,1);details.Controls.Add(new Label {Text=damage.ToString("N0")+" DAMAGE",Dock=DockStyle.Fill,Font=new Font("Segoe UI",25,FontStyle.Bold),ForeColor=Theme.Coral},0,2);int shownHP=defeated?0:currentHP;details.Controls.Add(new JourneyMeter {Maximum=Math.Max(1,maxHP),Value=shownHP,Accent=defeated?Theme.Gold:Theme.Coral,Dock=DockStyle.Fill,AccessibleName=boss+" health: "+shownHP+" of "+maxHP},0,3);string outcome=defeated?boss+" was defeated!\nNext encounter: "+nextBoss+" · "+currentHP+" HP":boss+" has "+currentHP+" / "+maxHP+" HP remaining.";if(rewards.Count>0)outcome+="\nReward: "+String.Join(", ",rewards);details.Controls.Add(new Label {Text=outcome,Dock=DockStyle.Fill,ForeColor=Theme.Muted,AutoEllipsis=true},0,4);var close=new ThemedButton {Text="Continue adventure",AutoSize=true,DialogResult=DialogResult.OK};Theme.StyleButton(close,true);var actions=new FlowLayoutPanel {Dock=DockStyle.Fill,FlowDirection=FlowDirection.RightToLeft};actions.Controls.Add(close);details.Controls.Add(actions,0,5);frame.Controls.Add(details,1,0);AcceptButton=close;CancelButton=close;}
 }
}
